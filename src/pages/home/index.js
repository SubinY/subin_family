...
import { AtActivityIndicator, AtNoticebar, AtButton, AtIcon } from 'taro-ui';
  ...
  render() {
    return (
      <View
        ...
        style={{
          'margin-bottom':
            !this.state.loading && this.currPage === this.totalPage
              ? '10px'
              : '50px'
        }}
      >
        ...
        <Swiper
          ...
          autoplay
        >
          {this.state.bannerInfo.map((b, index) => (
            <SwiperItem className='banner-item' key={index} onClick={this.previewBanner.bind(this, b.detailUrl)}>
              <Image
                className='banner-img'
                src={b.showUrl || product_image_default}
              />
            </SwiperItem>
          ))}
        </Swiper>
        <ScrollView
          className={`${this.state.isFixed ? 'fixed' : ''} scroll-view-x`}
          scrollX
          scrollWithAnimation
          scrollLeft='0'
        >
          <View class='title-wrap'>
            {this.state.tempCatalog.map((item, index) => {
              return (
                <View
                  key={index}
                  onClick={this.slideSelected.bind(this, item)}
                  className={`${
        ...
        {!this.state.loading &&
          ...
          this.currPage === this.totalPage && (
            <View className='no-more'>没得再拉啦！</View>
          )}
        <ExplainModal
        ...
        tempCatalog.forEach(el => {
          if (this.currCatalog === el.value) {
            ...
            this.setState({
              tempCatalog,
              list:
                this.currPage === 1 ? [...data] : [...this.state.list, ...data],
              loading: false
            });
          }
        });
