...
export default class GoodDetailPage extends Component {
      ...
      <View className='product-detail'>
        ...
        <View className='detail-content'>
          <View className='price-wrap'>
            ...
            {shopPrice && (
              <View className='origin-price'>网上价格:¥{shopPrice}</View>
            )}
            {description && (
              <View className='origin-price orange'>
                货物情况: {description}
              </View>
            )}
          </View>
          <View className='title-wrap'>
            ...
            {from && (
              <Image
                className='title-share-pic'
                src={center_link}
                onClick={this.copy.bind(this, from)}
              />
            )}
          </View>
        ...
        <AtToast
          ...
          text='已复制该商品的官网地址，请打开浏览器输入查看'
        />
        <AtCurtain
          isOpened={this.state.curtainOpen}
          onClose={this.onCurtainClose.bind(this)}
        >
          <Image
            style='width:100%; height: 400px'
            src={wxcode}
            onLongPress={this.previewimage}
          />
        </AtCurtain>
      </View>
    );
  ...
  getDetail() {
    post(`products_details/${this.state.goodId}`, {})
      .then(({ data }) => {
        this.setState({
          content: data
        });
